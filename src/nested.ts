...
export function addNewQuestion(
    ...
    const newQuestion = makeBlankQuestion(id, name, type); // Assume makeBlankQuestion is defined elsewhere
...
export function duplicateQuestionInArray(
    ...
    const index = newQuestions.findIndex((q) => q.id === targetId);

    if (index !== -1) {
        const originalQuestion = newQuestions[index];
